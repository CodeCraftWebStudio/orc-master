...
def get_user_by_key(session_key):
    try:
        ...
        user = User.query.filter_by(session_key=reversible_dehasher(
            session_key, SECRET_KEY), is_active=True).first()
        return user
        ...
        try:
            ...
            user = User.query.filter_by(
                session_key=session_key, is_active=True).first()
            return user
        except Exception as e:
...
def resign_role(user: User):
    if user.role_type == RoleType.ADMIN:
        raise PermissionError(
            "The President cannot resign unless they appoint another person to take their place.")
    user.role_type = RoleType.MEMBER
...
def check_user_registered_from_session_key(hashed_session_key, secret_key=SECRET_KEY):
    user = get_user_from_key(reversible_dehasher(
        hashed_session_key, secret_key))
    if not user:
...
def check_if_a_user_has_logged_in_with_a_role(given_role_title):
    answer = User.query.filter_by(
        role_title=given_role_title, is_active=True).first()
    if answer:
        return True
    return False
