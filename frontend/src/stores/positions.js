...
export async function GetCodes(
  ...
  const magicKeyResponse = await (
    await fetch(`${base}${magicKeyRoute}`, magicKeyConfig)
  ).json();
  ...
  const codeRouteConfig = {
    passcode: await (
      await fetch(`${base}${hashRoute}`, hashedKeyConfig)
    ).json(),
    session_key: localStorage.getItem("key"),
    position_name: position,
  };
  let isDisabled;
  let theFinalResult = await (
    await fetch(`${base}${codeRouteName}`, codeRouteConfig)
  ).json();
  localStorage.setItem("key", theFinalResult.session_key);
  if (
    theFinalResult.disabled_roles &&
    theFinalResult.diabled_roles.length &&
    theFinalResult.disabled_roles.includes(position)
  ) {
    isDisabled = true;
  } else {
    isDisabled = false;
...
export const positions = [
  makePosition(
    position_names.president,
    position_types.admin,
    position_codes.president,
  ),
  makePosition(
    position_names.vicePresident,
    position_types.edit,
    position_codes.vicePresident,
  ),
  makePosition(
    position_names.commandant,
    position_types.edit,
    position_codes.commandant,
  ),
  makePosition(
    position_names.secretary,
    position_types.edit,
    position_codes.secretary,
  ),
  makePosition(
    position_names.financialOfficer,
    position_types.financialOfficer,
    position_codes.financialOfficer,
  ),
  makePosition(
    position_names.technicalDirector,
    position_types.nonEdit,
    position_codes.technicalDirector,
  ),
  makePosition(
    position_names.socialDirector,
    position_types.nonEdit,
    position_codes.socialDirector,
  ),
  makePosition(
    position_names.assistantCommandant,
    position_types.nonEdit,
    position_codes.assistantCommandant,
  ),
  makePosition(
    position_names.assistantSecretary,
    position_types.nonEdit,
    position_codes.assistantSecretary,
  ),
  makePosition(
    position_names.assistantFinancialOfficer,
    position_types.nonEdit,
    position_codes.assistantFinancialOfficer,
  ),
  makePosition(
    position_names.assistantTechnicalDirector,
    position_types.nonEdit,
    position_codes.assistantTechnicalDirector,
  ),
  makePosition(
    position_names.assistantSocialDirector,
    position_types.nonEdit,
    position_codes.assistantSocialDirector,
  ),
  ...
  makePosition(
    position_names.outgoneAdvisor1,
    position_types.edit,
    position_codes.outgoneAdvisor1,
  ),
  makePosition(
    position_names.outgoneAdvisor2,
    position_types.edit,
    position_codes.outgoneAdvisor2,
  ),
];
